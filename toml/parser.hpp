...
parse_boolean(location<Container>& loc)
{
    ...
    if(const auto token = lex_boolean::invoke(loc))
    {
        ...
        {
            TOML_THROW(internal_error(format_underline(
...
parse_floating(location<Container>& loc)
{
    ...
    if(const auto token = lex_float::invoke(loc))
    {
        ...
        if(str == "inf" || str == "+inf")
        {
            ...
            {
                TOML_THROW(std::domain_error("toml::parse_floating: inf value found"
        ...
        else if(str == "nan" || str == "+nan")
        {
            ...
            {
                TOML_THROW(std::domain_error("toml::parse_floating: NaN value found"
              ...
              /* for err msg */ const location<Container2>& loc)
{
    ...
    else if(codepoint < 0x10000) // U+0800...U+FFFF
    {
        ...
        {
            TOML_THROW(syntax_error(format_underline(
    ...
    {
        TOML_THROW(syntax_error(format_underline("toml::read_utf8_codepoint:"
...
parse_basic_string(location<Container>& loc)
{
    ...
    if(const auto token = lex_basic_string::invoke(loc))
    {
        ...
        if(!quot)
        {
            TOML_THROW(internal_error(format_underline("parse_basic_string: "
...
parse_key(location<Container>& loc)
{
    ...
    if(const auto token = lex_dotted_key::invoke(loc))
    {
        ...
        while(inner_loc.iter() != inner_loc.end())
        {
            ...
            {
                TOML_THROW(internal_error(format_underline("toml::parse_key: "
    ...
    while(loc.iter() != loc.end())
    {
        ...
        if(auto val = parse_value<value_type>(loc))
        {
            ...
            if(!retval.empty() && retval.front().type() != val.as_ok().type())
            {
                ...
                TOML_THROW(syntax_error(format_underline("toml::parse_array: "
            ...
            if(is_array_of_table)
            {
                if(tab->count(k) == 1) // there is already an array of table
                {
                    if(tab->at(k).is_table())
                    {
                        ...
                        TOML_THROW(syntax_error(format_underline(concat_to_string(
...
parse_inline_table(location<Container>& loc)
{
    ...
    while(loc.iter() != loc.end())
    {
        ...
        if(!inserted)
        {
            TOML_THROW(internal_error("toml::parse_inline_table: "
    ...
    loc.reset(first);
    TOML_THROW(syntax_error(format_underline("toml::parse_inline_table: "
...
parse_array_table_key(location<Container>& loc)
{
    if(auto token = lex_array_table::invoke(loc))
    {
        ...
        if(loc.iter() != loc.end())
        {
            ...
            if(!nl)
            {
                TOML_THROW(syntax_error(format_underline("toml::"
...
parse(std::istream& is, const std::string& fname = "unknown file")
{
    ...
    if(!data)
    {
        TOML_THROW(syntax_error(data.unwrap_err(), source_location(std::addressof(loc))));
    }
    return data.unwrap();
}
...
basic_value<Comment, Table, Array> parse(const std::string& fname)
{
    ...
    if(!ifs.good())
    {
        TOML_THROW(std::runtime_error("toml::parse: file open error -> " + fname));
    }
